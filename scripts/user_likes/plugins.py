...
def main():
    ...
    query_string = (
        f"repo:{repository.full_name} is:issue is:open label:\"potential plugin\""
    )

    issue_data_list = [IssueData(issue) for issue in github.search_issues(query_string)]
    plugin_request_issue = IssueData(repository.get_issue(number=388))
    issue_data_list.append(plugin_request_issue)
